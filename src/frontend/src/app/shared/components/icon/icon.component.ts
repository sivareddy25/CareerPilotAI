...
export type IconName =
  | 'search' | 'user' | 'check' | 'x' | 'chevron-down' | 'chevron-up' | 'chevron-left' | 'chevron-right'
  | 'bell' | 'sun' | 'moon' | 'monitor' | 'settings' | 'filter' | 'trash' | 'edit' | 'calendar' | 'upload'
  | 'info' | 'warning' | 'error' | 'command' | 'menu' | 'more-vertical' | 'external-link' | 'grid' | 'list'
  | 'file-text' | 'plus' | 'download' | 'share' | 'eye' | 'eye-off' | 'lock' | 'mail' | 'phone'
  | 'arrow-right' | 'arrow-left' | 'sparkles' | 'briefcase' | 'building' | 'dollar-sign' | 'map-pin'
  | 'check-circle' | 'x-circle' | 'alert-triangle' | 'refresh-cw' | 'columns' | 'copy' | 'help-circle';
...
export class IconComponent {
  ...
  private readonly sanitizer = computed(() => {
    const rawPath = ICON_PATHS[this.name()] || ICON_PATHS['help-circle'];
    return rawPath;
  });

  protected readonly svgContent = computed<SafeHtml>(() => {
    // Basic inner HTML mapping
    return this.name() in ICON_PATHS ? ICON_PATHS[this.name()] : ICON_PATHS['help-circle'];
  });

  protected iconClasses = computed(() => {
    return `icon-${this.size()} ${this.class()}`;
  });
}
